...
class Optimizer():
	...
	def infer_shapes(self, return_onnx=False):
		...
		if onnx_graph.ByteSize() > 2147483648:
			...
			onnx.save_model(onnx_graph, onnx_orig_path, save_as_external_data=True, all_tensors_to_one_file=True, convert_attribute=False)
...
class BaseModel():
	def __init__(self, version="1.5", pipeline=None, hf_token="", device="cuda", verbose=True, fp16=False, max_batch_size=16, text_maxlen=77, embedding_dim=768, controlnet=None):
...
class CLIP(BaseModel):
	def __init__(self, version, pipeline, hf_token, device, verbose, max_batch_size, embedding_dim, output_hidden_states=False, subfolder="text_encoder"):
	...
	def get_shape_dict(self, batch_size, image_height, image_width):
		self.check_dims(batch_size, image_height, image_width)
		output = {"input_ids": (batch_size, self.text_maxlen), "text_embeddings": (batch_size, self.text_maxlen, self.embedding_dim)}
...
class CLIPWithProj(CLIP):
	def __init__(self, version, pipeline, hf_token, device="cuda", verbose=True, max_batch_size=16, output_hidden_states=False, subfolder="text_encoder_2"):
	...
	def get_shape_dict(self, batch_size, image_height, image_width):
		self.check_dims(batch_size, image_height, image_width)
		output = {"input_ids": (batch_size, self.text_maxlen), "text_embeddings": (batch_size, self.embedding_dim)}
		if "hidden_states" in self.extra_output_names:
			output["hidden_states"] = (batch_size, self.text_maxlen, self.embedding_dim)
...
class UNet2DConditionControlNetModel(torch.nn.Module):
	...
	def forward(self, sample, timestep, encoder_hidden_states, images, controlnet_scales):
		for i, (image, conditioning_scale, controlnet) in enumerate(zip(images, controlnet_scales, self.controlnets)):
			down_samples, mid_sample = controlnet(sample, timestep, encoder_hidden_states=encoder_hidden_states, controlnet_cond=image, return_dict=False)
		...
		noise_pred = self.unet(sample, timestep, encoder_hidden_states=encoder_hidden_states, down_block_additional_residuals=down_block_res_samples, mid_block_additional_residual=mid_block_res_sample)
		return noise_pred

class UNet(BaseModel):
	def __init__(self, version, pipeline, hf_token, device="cuda", verbose=True, fp16=False, max_batch_size=16, text_maxlen=77, unet_dim=4, controlnet=None):
	...
	def get_model(self, framework_model_dir):
		...
		if self.controlnet:
			unet_model = UNet2DConditionModel.from_pretrained(self.path, subfolder="unet", use_safetensors=self.hf_safetensor, use_auth_token=self.hf_token, **model_opts).to(self.device)
			...
			if not os.path.exists(unet_model_dir):
				model = UNet2DConditionModel.from_pretrained(self.path, subfolder="unet", use_safetensors=self.hf_safetensor, use_auth_token=self.hf_token, **model_opts).to(self.device)
...
def make_UNet(version, pipeline, hf_token, device, verbose, max_batch_size, controlnet=None):
	...
	return UNet(version, pipeline, hf_token, fp16=True, device=device, verbose=verbose, max_batch_size=max_batch_size, unet_dim=(9 if pipeline.is_inpaint() else 4), controlnet=get_controlnets_path(controlnet))

class UNetXL(BaseModel):
	def __init__(self, version, pipeline, hf_token, fp16=False, device="cuda", verbose=True, max_batch_size=16, text_maxlen=77, unet_dim=4, time_dim=6):
	...
	def get_input_profile(self, batch_size, image_height, image_width, static_batch, static_shape):
		latent_height, latent_width = self.check_dims(batch_size, image_height, image_width)
		min_batch, max_batch, _, _, _, _, min_latent_height, max_latent_height, min_latent_width, max_latent_width = self.get_minmax_dims(batch_size, image_height, image_width, static_batch, static_shape)
		return {
			"sample": [
				(2*min_batch, self.unet_dim, min_latent_height, min_latent_width),
				(2*batch_size, self.unet_dim, latent_height, latent_width),
...
def make_UNetXL(version, pipeline, hf_token, device, verbose, max_batch_size):
	...
	if hasattr(F, "scaled_dot_product_attention"):
		delattr(F, "scaled_dot_product_attention")
	return UNetXL(version, pipeline, hf_token, fp16=True,  device=device, verbose=verbose, max_batch_size=max_batch_size, unet_dim=4, time_dim=(5 if pipeline.is_sd_xl_refiner() else 6))
...
class VAE(BaseModel):
	...
	def get_model(self, framework_model_dir):
		...
		if not os.path.exists(vae_decoder_model_path):
			vae = AutoencoderKL.from_pretrained(self.path, subfolder="vae", use_safetensors=self.hf_safetensor, use_auth_token=self.hf_token).to(self.device)
...
class VAEEncoder(BaseModel):
	...
	def get_input_profile(self, batch_size, image_height, image_width, static_batch, static_shape):
		...
		max_batch = batch_size if static_batch else self.max_batch
		self.check_dims(batch_size, image_height, image_width)
		min_batch, max_batch, min_image_height, max_image_height, min_image_width, max_image_width, _, _, _, _ = self.get_minmax_dims(batch_size, image_height, image_width, static_batch, static_shape)
...
def make_tokenizer(version, pipeline, hf_token, framework_model_dir, subfolder="tokenizer"):
	...
	if not os.path.exists(tokenizer_model_dir):
		model = CLIPTokenizer.from_pretrained(get_path(version, pipeline), subfolder=subfolder, use_safetensors=pipeline.is_sd_xl(), use_auth_token=hf_token)
