...
import onnx
import torch
from utilities import Engine, PIPELINE_TYPE, save_image, DPMScheduler, DDIMScheduler, EulerAncestralDiscreteScheduler, LMSDiscreteScheduler, PNDMScheduler, UniPCMultistepScheduler
import pathlib

class StableDiffusionPipeline:
	"""Application showcasing the acceleration of Stable Diffusion pipeline using NVidia TensorRT."""
	def __init__(
			self,
			version="2.1",
		...
		for model_name, obj in self.models.items():
			...
			if force_build or not os.path.exists(engine.engine_path):
				...
				engine.build(
					onnx_opt_path,
					fp16=True,
					input_profile=obj.get_input_profile(opt_batch_size, opt_image_height, opt_image_width, static_batch=static_batch, static_shape=static_shape),
	...
	def encode_prompt(self, prompt, negative_prompt, encoder="clip", tokenizer=None, pooled_outputs=False, output_hidden_states=False):
		...
		# Tokenize prompt
		text_input_ids = tokenizer(prompt, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt").input_ids.type(torch.int32).to(self.device)
		...
		uncond_input_ids = tokenizer(negative_prompt, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt").input_ids.type(torch.int32).to(self.device)
