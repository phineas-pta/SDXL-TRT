...
from polygraphy.backend.trt import CreateConfig, ModifyNetworkOutputs, Profile, engine_from_bytes, engine_from_network, network_from_onnx_path, save_engine
import random
...
class LMSDiscreteScheduler():
	def __init__(self, device="cuda", beta_start=85e-5, beta_end=0.012, num_train_timesteps=1000, steps_offset=0, prediction_type="epsilon"):
	...
	def step(self, output, latents, idx, timestep):
		...
		prev_sample = latents + sum(coeff * derivative for coeff, derivative in zip(self.lms_coeffs[idx], reversed(self.derivatives)))
...
class DDIMScheduler():
	...
	def step(self, model_output, sample, idx, timestep, eta: float = 0.0, use_clipped_model_output:bool=False, generator=None, variance_noise: torch.FloatTensor=None):
...
class EulerAncestralDiscreteScheduler():
	def __init__(self, num_train_timesteps:int=1000, beta_start:float=1e-4, beta_end:float=.02, device="cuda", steps_offset=0, prediction_type="epsilon"):
		# this schedule is very specific to the latent diffusion model.
		betas = torch.linspace(beta_start**0.5, beta_end**0.5, num_train_timesteps, dtype=torch.float32) ** 2
		alphas = 1.0 - betas
		self.alphas_cumprod = torch.cumprod(alphas, dim=0)
	...
	def step(self, model_output, sample, idx, timestep, generator = None):
		step_index = (self.timesteps == timestep).nonzero().item()
		sigma = self.sigmas[step_index]
...
class DPMScheduler():
	...
	def set_timesteps(self, num_inference_steps):
		self.num_inference_steps = num_inference_steps
		timesteps = np.linspace(0, self.num_train_timesteps - 1, num_inference_steps + 1).round()[::-1][:-1].copy().astype(np.int32)
		self.timesteps = torch.from_numpy(timesteps).to(self.device)
		self.model_outputs = [None,] * self.solver_order
		self.lower_order_nums = 0

	def convert_model_output(self, model_output, timestep, sample):
		...
		if self.algorithm_type == "dpmsolver++":
			if self.prediction_type == "epsilon":
				...
				raise ValueError(f"prediction_type given as {self.prediction_type} must be one of `epsilon`, or `v_prediction` for the DPMScheduler.")

			if self.thresholding:
				# Dynamic thresholding in https://arxiv.org/abs/2205.11487
				dynamic_max_val = torch.quantile(torch.abs(x0_pred).reshape((x0_pred.shape[0], -1)), self.dynamic_thresholding_ratio, dim=1)
				dynamic_max_val = torch.maximum(dynamic_max_val, self.sample_max_value * torch.ones_like(dynamic_max_val).to(dynamic_max_val.device))[(...,) + (None,) * (x0_pred.ndim - 1)]
	...
	def dpm_solver_first_order_update(self, idx, model_output, sample):
	...
	def multistep_dpm_solver_second_order_update(self, idx, model_output_list, timestep_list, prev_timestep, sample):
		t, s0, s1 = prev_timestep, timestep_list[-1], timestep_list[-2]
		...
		if self.algorithm_type == "dpmsolver++":
			# See https://arxiv.org/abs/2211.01095 for detailed derivations
			if self.solver_type == "midpoint":
				x_t = first_coef * sample - second_coef * D0 - third_coef * D1
			elif self.solver_type == "heun":
				x_t = first_coef * sample - second_coef * D0 + third_coef * D1
		elif self.algorithm_type == "dpmsolver":
			# See https://arxiv.org/abs/2206.00927 for detailed derivations
	...
	def multistep_dpm_solver_third_order_update(self, idx, model_output_list, timestep_list, prev_timestep, sample):
		...
		if self.algorithm_type == "dpmsolver++":
			# See https://arxiv.org/abs/2206.00927 for detailed derivations
			x_t = first_coef * sample - second_coef * D0 + third_coef * D1 - fourth_coef * D2
		elif self.algorithm_type == "dpmsolver":
			# See https://arxiv.org/abs/2206.00927 for detailed derivations
			x_t = first_coef * sample - second_coef * D0 - third_coef * D1 - fourth_coef * D2
		return x_t
	...
	def step(self, output, latents, step_index, timestep):
		...
		lower_order_final = (step_index == len(self.timesteps) - 1) and self.lower_order_final and len(self.timesteps) < 15
		lower_order_second = (step_index == len(self.timesteps) - 2) and self.lower_order_final and len(self.timesteps) < 15
		...
		if self.solver_order == 1 or self.lower_order_nums < 1 or lower_order_final:
			...
			timestep_list = [self.timesteps[step_index - 1], timestep]
			prev_sample = self.multistep_dpm_solver_second_order_update(step_index, self.model_outputs, timestep_list, prev_timestep, latents)
		else:
			timestep_list = [self.timesteps[step_index - 2], self.timesteps[step_index - 1], timestep]
			prev_sample = self.multistep_dpm_solver_third_order_update(step_index, self.model_outputs, timestep_list, prev_timestep, latents)
...
class PNDMScheduler():
	def __init__(self, device="cuda", beta_start=85e-5, beta_end=0.012, num_train_timesteps=1000, steps_offset:int=0, prediction_type="epsilon"):
		self.device = device
		self.num_train_timesteps = num_train_timesteps
		self.pndm_order = 4
	...
	def configure(self):
		...
		self.model_output_denom_coeff = self.alphas_cumprod * (self.beta_cumprod_prev) ** (0.5) + (self.alphas_cumprod * self.beta_cumprod * self.alphas_cumprod_prev) ** (0.5)
	...
	def step(self, output, sample, idx, timestep):
		...
		prev_sample = self.sample_coeff[idx] * sample - (self.alphas_cumprod_prev[idx] - self.alphas_cumprod[idx]) * output / self.model_output_denom_coeff[idx]
		self.counter += 1
...
class UniPCMultistepScheduler():
	...
	def multistep_uni_p_bh_update(self, model_output: torch.FloatTensor, prev_timestep: int, sample: torch.FloatTensor, order: int) -> torch.FloatTensor:
	...
	def multistep_uni_c_bh_update(self, this_model_output: torch.FloatTensor, this_timestep: int, last_sample: torch.FloatTensor, this_sample: torch.FloatTensor, order: int) -> torch.FloatTensor:
	...
	def step(self, model_output: torch.FloatTensor, timestep: int, sample: torch.FloatTensor, return_dict: bool=True):
		...
		if use_corrector:
			sample = self.multistep_uni_c_bh_update(this_model_output=model_output_convert, this_timestep=timestep, last_sample=self.last_sample, this_sample=sample, order=self.this_order)
		...
		prev_sample = self.multistep_uni_p_bh_update(model_output=model_output, prev_timestep=prev_timestep, sample=sample, order=self.this_order)
